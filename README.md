...
4. 打开桌面的 **ECARX 保电修复**，或从 LSPosed 的模块设置入口打开设置页，独立选择“启动后切换 HEV”“启动后关闭低速行驶提示”，设置共享启动延时；在“空调内外循环”中按需启用“纠正 AQS／G-clean 引起的内外循环变化”（默认关闭），点击 **保存设置**。
5. 安装／升级后重启车机，使目标进程重新加载模块。两个开机功能开关默认开启，共享开机延时默认 5 秒，保存后下次车机重启生效。空调纠正开关默认关闭；模块已加载且空调进程运行时，保存后约 1 秒内生效，不需要再次重启。
...
| 设置 | 默认值 | 作用 |
| --- | --- | --- |
| 启动后切换 HEV | 开启 | 延时结束后先确认 HEV，再恢复保电；不控制低速提示任务是否启用 |
| 启动后关闭低速行驶提示 | 开启 | 独立关闭低速提示，不要求 HEV 选项开启或保电恢复成功 |
| 开机执行延时 | 5 秒 | 5–300 秒，滑块按 1 秒调节，提供 5、30、60、180、300 秒快捷按钮 |
| 纠正 AQS／G-clean 引起的内外循环变化 | 关闭 | 保存后约 1 秒内生效，与开机任务及共享延时独立；不控制文件接口是否可用 |

延时从**本次开机第一次 `ecarx.settings` 启动**开始计时，两个开关的组合如下：

| 切换 HEV | 关闭低速提示 | 延时结束后的行为 |
| --- | --- | --- |
| 关闭 | 关闭 | 不强制 HEV、不自动关闭低速提示；保电继续按当前模式恢复 |
| 开启 | 关闭 | 切换 HEV，然后恢复保电；不自动关闭低速提示 |
| 关闭 | 开启 | 独立关闭低速提示；保电按当前模式恢复，EV 等条件不阻塞低速提示 |
| 开启 | 开启 | 正常情况下先 HEV、再保电、再关闭低速提示；前两项等待、失败或取消时，低速提示独立继续 |

两项都开启且车辆条件满足时，保持以下顺序：
...
- 两项功能**只共用一次启动延时**，不为低速提示再等待一轮 5–300 秒。HEV 选项关闭时，低速提示在延时结束后独立执行；两项均开启时优先按正常顺序执行，但 HEV 等待车辆条件、保电在 EV／非舒适模式中等待、回读失败或被用户取消时，低速提示任务仍可继续，不以其他任务成功为前提。
...
- HEV 请求发送后至少等待 1 秒确认，最多发送 **2 次（首次加一次重试）**。延时结束后最多主动等待车辆条件 60 秒；HEV 未确认或条件超时时，停止这一轮 HEV／保电恢复，低速提示任务继续独立运行。共享配置无法读取时不猜测开关值，跳过自动任务。
- 保电开启仍要求 HEV/SAVE 和舒适模式；不改动驾驶模式。保电开启和关闭都补发原应用保存的 30–85% 目标；HOLD 保持原有含义。
- 手动切换 EV/HEV、保电或调节保电目标，以及有效的 EV/HEV、保电文件指令，只取消对应的 HEV／保电恢复，不取消低速提示任务。手动或文件控制低速提示，只取消其自动关闭，不中断 HEV／保电恢复。模块自己的请求不会被当作手动操作，两项任务不共用重试次数或完成状态。
...
设置页“空调内外循环”下的 **“纠正 AQS／G-clean 引起的内外循环变化”默认关闭**，新装和从没有此设置的旧版本升级都不会自动启用。设置通过现有只读 ContentProvider 提供给空调进程；每轮同步重新读取，保存后约 1 秒内生效，配置不可读或缺少该键时按关闭处理。关闭会停止未完成的自动纠正，并跟随当前实际模式建立基线，重新开启时不拉回关闭期间的旧选择。**开关只控制自动纠正，文件手动控制及实际状态输出始终独立工作**，也不使用开机的 HEV／低速提示延时。
